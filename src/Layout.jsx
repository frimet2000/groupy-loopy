...
function LayoutContent({ children, currentPageName }) {
  ...
  const allNavItems = [
        ...
        { name: 'Copilot', icon: MessageSquare, label: language === 'he' ? 'קופילוט' : language === 'ru' ? 'Копилот' : language === 'es' ? 'Copiloto' : language === 'fr' ? 'Copilote' : language === 'de' ? 'Copilot' : language === 'it' ? 'Copilota' : 'Copilot', color: 'text-emerald-700' },
