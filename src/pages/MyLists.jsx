        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              {language === 'he' ? 'ארגן את הטיולים שלך ברשימות מותאמות אישית' : 'Organize your trips in custom lists'}
            </p>
          </div>
          <Button
        ...
        {isLoading ? (
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <Badge variant="outline" className="gap-1">
                          ...
                          {listTrips.length} {language === 'he' ? 'טיולים' : 'trips'}
                        </Badge>
                      </div>
          ...
          <Card className="p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">
              {language === 'he' ? 'אין לך רשימות עדיין' : 'No lists yet'}
            </h3>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingList 
                ...
                : (language === 'he' ? 'צור רשימה חדשה' : 'Create New List')}
            </DialogTitle>
            <DialogDescription>
              {language === 'he' 
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>{language === 'he' ? 'שם הרשימה' : 'List Name'}</Label>
              <Input
                value={formData.name}
                onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                placeholder={language === 'he' ? 'למשל: טיולים שחייבים לעשות' : 'e.g., Must-do trips'}
                dir={isRTL ? 'rtl' : 'ltr'}
              />
            </div>
            <div className="space-y-2">
              <Label>{language === 'he' ? 'תיאור (אופציונלי)' : 'Description (optional)'}</Label>
              <Textarea
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                placeholder={language === 'he' ? 'תאר את הרשימה...' : 'Describe your list...'}
                rows={3}
                dir={isRTL ? 'rtl' : 'ltr'}
              />
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
              <Label className="mb-0">
                {language === 'he' ? 'רשימה ציבורית' : 'Public List'}
              </Label>
              <Switch
          ...
          <DialogFooter>
            ...
            >
              ...
              {editingList ? t('save') : (language === 'he' ? 'צור' : 'Create')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
