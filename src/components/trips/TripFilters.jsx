      ...
      <div className="p-3 sm:p-4">
        ...
        <div className="flex flex-col md:flex-row gap-3 sm:gap-4 items-stretch md:items-center justify-between">
          <div className="relative w-full md:flex-1 md:max-w-md">
            ...
            <Input
              ...
              className={`${isRTL ? 'pr-9' : 'pl-9'} bg-white border-emerald-200 focus:border-emerald-400 transition-all h-11 sm:h-10 text-base sm:text-sm`}
            />
          </div>
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
             <Button
              ...
              className="gap-2 flex-1 md:flex-initial border-2 border-emerald-300 hover:bg-emerald-50 text-emerald-700 font-semibold h-11 sm:h-10 text-sm sm:text-base shadow-sm"
            >
              ...
              {activeFiltersCount > 0 && (
                <Badge variant="secondary" className="ml-1 bg-emerald-600 text-white hover:bg-emerald-700 shadow-sm">
                  {activeFiltersCount}
                </Badge>
              )}
            </Button>
            ...
            {activeFiltersCount > 0 && (
              <Button
                ...
                className="text-gray-500 hover:text-red-600 hover:bg-red-50 h-11 sm:h-10 w-11 sm:w-10"
                size="icon"
              >
        ...
        {activeFiltersCount > 0 && !isOpen && (
          <div className="flex flex-wrap gap-2 mt-3">
             {filters.country && (
               <Badge variant="outline" className="gap-1 bg-white border-emerald-300 text-emerald-700">
                 {t(filters.country)}
                 <X className="w-3 h-3 cursor-pointer hover:text-red-600" onClick={() => handleFilterChange('country', '')} />
               </Badge>
             )}
             {filters.region && (
               <Badge variant="outline" className="gap-1 bg-white border-blue-300 text-blue-700">
                 {t(filters.region)}
                 <X className="w-3 h-3 cursor-pointer hover:text-red-600" onClick={() => handleFilterChange('region', '')} />
               </Badge>
             )}
             {filters.difficulty && (
               <Badge variant="outline" className="gap-1 bg-white border-purple-300 text-purple-700">
                 {t(filters.difficulty)}
                 <X className="w-3 h-3 cursor-pointer hover:text-red-600" onClick={() => handleFilterChange('difficulty', '')} />
               </Badge>
             )}
          </div>
        )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    ...
                    <div className="space-y-3">
                      <div>
                        <Label className="text-xs text-gray-500 mb-1.5 block font-medium">{t('country')}</Label>
                      ...
                      {filters.country && getCountryRegions(filters.country).length > 0 && (
                        <div>
                          <Label className="text-xs text-gray-500 mb-1.5 block font-medium">{t('region')}</Label>
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    <div className="flex items-center gap-2 text-blue-700 font-semibold text-sm sm:text-base">
                    ...
                    <div className="space-y-3">
                       <div>
                        <Label className="text-xs text-gray-500 mb-1.5 block font-medium">{t('difficulty')}</Label>
                      ...
                      <div>
                        <Label className="text-xs text-gray-500 mb-1.5 block font-medium">{t('duration')}</Label>
                  ...
                  {/* Type & Preferences */}
                  <div className="space-y-3 sm:space-y-4">
                    <div className="flex items-center gap-2 text-purple-700 font-semibold text-sm sm:text-base">
                    ...
                    <div className="space-y-3">
                      <div>
                        <Label className="text-xs text-gray-500 mb-1.5 block font-medium">{t('activityType')}</Label>
                      ...
                      <div className="flex flex-col gap-2 pt-1">
                        <div className="flex items-center space-x-2 rtl:space-x-reverse">
                          ...
                          />
                          <label htmlFor="available_spots" className="text-xs sm:text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer text-gray-600">
                        ...
                        <div className="flex items-center space-x-2 rtl:space-x-reverse">
                          <Checkbox 
                            ...
                            className="touch-manipulation"
                          />
                          <label htmlFor="favorites" className="text-xs sm:text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer text-gray-600">
                   ...
                   {/* Tags Section */}
                   <div className="space-y-3 sm:space-y-4">
                    <div className="flex items-center gap-2 text-orange-700 font-semibold text-sm sm:text-base">
                    ...
                    <div className="flex flex-wrap gap-1.5 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                      {trailTypes.map(type => (
                        <Badge 
                          ...
                          className={`cursor-pointer touch-manipulation transition-all ${filters.trail_type.includes(type) ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm' : 'hover:bg-emerald-50 text-gray-600 border-gray-300'}`}
                          onClick={() => {
