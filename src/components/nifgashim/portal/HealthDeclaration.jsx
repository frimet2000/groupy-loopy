...
const translations = {
  he: {
    ...
    subtitle: 'אנא קרא בעיון מלא את כל המסמך. זה מסמך משפטי חייב.',
    section1: 'הצהרת בריאות וכושר פיזי',
    section1Text: `בתחתום זה, אני מצהיר/ה בחתימתי כי:

א) אני בריא/ה ובעל/ת כושר פיזי כדי להשתתף בטיול זה בכל הפעילויות שלו.
ב) אני בן/בת הגיל המינימום הנדרש וגם לא בן/בת גיל שיוצר הגבלות רפואיות.
ג) לא קיימים בגופי מצבים רפואיים כולל: מחלות לב, לחץ דם גבוה, סוכרת, אסתמה, אפילפסיה או כל מצב רפואי אחר המעלה סיכון.
ד) אני לא נוטל/ת תרופות המעלות סיכון להשתתפות בפעילויות פיזיות אינטנסיביות.
ה) לא עברתי/תי ניתוח בחודשים האחרונים ואין לי פגיעה או פציעה פעילה.
ו) זה השיקול הרפואי שלי כי יכול/ה אני להשתתף בטיול זה בבטיחות מלאה.
ז) אני מאשר/ת כי כל המידע שנתתי עומד בתוקף ואני אחראי/ת לכל מידע שגוי או מסתור.`,
    
    section2: 'הבנה מלאה של הסיכונים',
    section2Text: `אני מודע/ת ומודעת היטב לעובדה כי השתתפות במסע זה כוללת סיכונים גופניים משמעותיים וניכרים, כולל אך לא מוגבל ל:
- פציעות גופניות קשות או קלות (שברים, חבלות, נקעים, חתכים, טראומות)
- מחלות כגון התגובות אלרגיות, דלקות, הידרציה יתר או התחממות
- הצטננות וגרדום, דום לב או כשל כלילי
- אובדן חיים
- שיתוק חלקי או מלא
- פגיעה במוח או בעמוד השדרה

אני מבין/ה כי סיכונים אלה עלולים להיות קטלניים, כולל מוות.`,
    
    section3: 'קבלה מלאה של הסיכונים',
    section3Text: `אני לוקח/ת על עצמי באופן מלא, בהחלט ובתודעה מלאה את כל הסיכונים הגלומים בהשתתפותי במסע זה. אני מודע/ת שלא ניתן למנוע לחלוטין סיכונים אלה גם עם טיפול רפואי טוב ביותר ומדריכים מנוסים. אני בוחר/ת להשתתף בהסכמה מלאה שלי עם הידיעה של כל הסיכונים הללו.`,
    
    section4: 'משחרור אחריות וחסינות משפטית',
    section4Text: `בחתימתי על מסמך זה, אני משחרר/ת ומשחרר/ת באופן מלא, בלתי מותנה ובשכל ישר:

1. את מארגן/ת הטיול ויוצר/ת התוכנית ל-Groupy Loopy
2. את Groupy Loopy ובעלי זכויות השדרות שלה וכל אחד מהם
3. את מדריכי הטיול וכל הצוות הטיפול והעובדים
4. את רופאים חירום או שירותי חירום שיוקרא להם
5. את המדינה, מחוז או קהל שקיים החוק בתחומו
6. כל צד שלישי שקשור בכל דרך לטיול

מכל דרישות, תביעות, הוצאות, קנסות, ריבית, הוצאות משפטיות ונזקים (כולל נזקים פסיכולוגיים, עדויות של שם קשה, אובדן הנאות חיים וניתוק קשרים משפחתיים) שאולי יחויבו עלי או על יורשיי כתוצאה מכל:
- פציעה, מחלה, נכות או מוות המתרחק במהלך הטיול או כתוצאה מהטיול
- נזק לרכוש או הפסד כספי
- הפרעה או הפסקה של הטיול מכל סיבה
- התנהגות או הערות של משתתפים אחרים או צוות`,
    
    section5: 'אחריות בלעדית של מארגנים',
    section5Text: `מארגני הטיול, Groupy Loopy, יוצרי האתר וכל בעלי זכויותיהם אינם נושאים בשום אחריות:
- לפציעות, מחלות, נכות או מוות שקרו או עלולים להיתקל במהלך הטיול
- לנזקים או הפסדים בעקבות הטיול
- לטעויות רפואיות שנעשו על ידי צוות טיפול או ניתוח
- לנתינת טיפול בחירום או השהיית טיפול
- לתנאי מזג אוויר או אירועים טבעיים
- לשיתוץ לחלקי על ידי צוות או משתתפים אחרים

הם אינם מחייבים בשום דרך לבדוק, לחקור או להודיע על מצב בריאותי שלך. אתה בלבד אחראי/ת לבדיקת בריאותך.`,
    
    section6: 'הסכמה להוצאה בפומבי של מידע אישי',
    section6Text: `אני מסכימ/ה שמטלם אישי שלי כולל שם, תמונה, תאריך לידה, ערכות חירום וכל מידע רפואי שסיפקתי עלול להיות מופץ או שימוש בצוות הטיול ורופאים חירום בנסיבות חירום.`,
    
    section7: 'ביטול ברצון חופשי',
    section7Text: `אני מצהיר/ה שהחתימה שלי על מסמך זה היא ברצוני הטוב, בהסכמה מלאה, וללא כל לחץ, איום או כפייה. אני קראתי את המסמך בעיון, הבנתי את כל התוכן שלו, ויש לי הזדמנות לשאול שאלות על כל חלק בו. אני מודע/ת לנימוקים המשפטיים של מסמך זה.`,
    
    agreeConfirm: 'אני מאשר/ת שקראתי את הצהרת הבריאות כולה וההסכם לשחרור אחריות, הבנתי את כל התוכן, וההסכמה שלי היא ברצוני החופשי',
